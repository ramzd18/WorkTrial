...
class AudioFeatureExtractor:
    ...
    def extract_snr(self, audio):
        ...
        audio_db = librosa.amplitude_to_db(torch.abs(audio).numpy())
    ...
    def extract_spectral_features(self, audio):
        ...
        mfcc_mean = torch.mean(mfccs_tensor, dim=1)  # Shape: (n_mfcc,)
        mfcc_std = torch.std(mfccs_tensor, dim=1)    # Shape: (n_mfcc,)
        
        # Calculate deltas and their statistics
        mfcc_delta = torch.tensor(librosa.feature.delta(mfccs))
        mfcc_delta2 = torch.tensor(librosa.feature.delta(mfccs, order=2))
        
        delta_mean = torch.mean(mfcc_delta, dim=1)    # Shape: (n_mfcc,)
        delta2_mean = torch.mean(mfcc_delta2, dim=1)  # Shape: (n_mfcc,)
        
        # Spectral features with fixed parameters
        ...
        # Calculate statistics across time to get fixed-size features
        features = {
    ...
    def extract_all_features(self, audio, sr):
        ...
        # print("Reverberation Slope")
        # features['envelope_kurtosis'] = env_kurtosis
        # print("Envelope Kurtosis")
